<?php
	...
	*/
	...
	$Columnas = array("Cliente","Comprobante De Ingreso","Creacion De Comprobante","Pieza","barcode","Sucursal","Sucursales","Division","Producto","Marca");
	$Consulta= "
		select  distinct(TU.Contador) as 'Contador', TU.Pieza as 'Pieza' , TU.barcode as 'barcode', TU.CDI as 'Comprobante De Ingreso', TU.CDC as 'Creacion De Comprobante'
		, TU.Cliente as 'Cliente'
		, Banco.Tarjeta
		...
		(
			...
			inner join sispoc5_gestionpostal.flash_clientes as clientes on ci.cliente_id = clientes.id
			...
			)as pe on pe.idPieza = fp.id and pe.Activo = '1' and pe.idEstados in ('17')
			where fp.tipo_id = 2
			and ci.cliente_id = '30'
			and 
			#(fp.create >  DATE_ADD('2020-01-01', INTERVAL 0 HOUR)  AND fp.create <  DATE_ADD('2021-01-01', INTERVAL 0 HOUR) )
			(fp.create >  DATE_ADD('$FechaI', INTERVAL 0 HOUR)  AND fp.create <  DATE_ADD('$FechaF', INTERVAL 0 HOUR) )
		) as TU
		inner join 
		(
