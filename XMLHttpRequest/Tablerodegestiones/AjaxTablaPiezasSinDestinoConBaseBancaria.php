<?php
	...
	*/
	...
	$Columnas = array("Cliente","Comprobante De Ingreso","Creacion De Comprobante","Pieza","barcode");
	$Consulta= "
		SELECT p.NumPiezaCorreo as 'Pieza', p.MarcaTarjeta as 'Marca', p.TipoTarjeta as 'Tipo'
		, p.FechaEstadoPieza as 'Fecha Ultimo Estado', p.EstadoPieza as 'Estado Actual'
		, p.ApellidoYNombre as 'Destinatario', p.DNI as 'DNI', p.Domicilio as 'Domicilio', p.Ciudad as 'Ciudad', p.Provincia as 'Provincia'
		, p.NumPiezaBanco as 'Numero De Pieza PAra Banco', p.sucursale_id as 'Sucursal Bancaria', p.SucursalRadicacion as 'Sucursal De Cuenta'
			FROM
			(
                SELECT SUBSTRING(TI.Telefono,1,2) as 'Destino' , TI.NumeroDeTarjeta as 'Barcode'
                ...
                (
                    ...
                    #DatoBanco
                ) as scw2 ON TI.NumeroDeTarjeta = scw2.NumPiezaBanco
                WHERE 1

                UNION

                SELECT TD.DestinoSegunCodigoDeEmision  as 'Destino' , TD.Tarjeta as 'Barcode'
                ...
                (
                    ...
                    WHERE (scw.MotivoPieza like '' and scw.DatoBanco like '')
                    #DatoBanco
                ) as scw2 ON TD.Tarjeta = scw2.NumPiezaBanco 
                WHERE 1

                UNION

                SELECT TSCE.Distribucion as 'Destino' , TSCE.SecuenciaDeTargeta as 'Barcode'
			...
			inner join sispoc5_consultasweb.piezas as p on CU.Barcode = p.NumPiezaBanco COLLATE utf8_unicode_ci
			where 1
			order by p.NumPiezaCorreo asc
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
