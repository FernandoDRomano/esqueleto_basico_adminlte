<?php
	...
	*/
	...
	$Columnas = array("barcode","Marca","Sucursal","Sucursales","Division","Producto"
	,"Destinatario","Domicilio","CodigoPostal","Localidad","Destino");
	$Consulta= "
		select  distinct(Banco.Tarjeta) as 'barcode'
		, Banco.Tarjeta
		...
		, Banco.Destinatario as 'Destinatario'
		, Banco.Domicilio as 'Domicilio'
		, Banco.CodigoPostal as 'CodigoPostal'
		, Banco.Localidad as 'Localidad'
		, Banco.Destino as 'Destino'
...
,Banco.Nombre as 'Destinatario'
,case 
WHEN Banco.Grupo like 'ANSES%' THEN Sucur.Domicilio
ELSE Banco.Domicilio
END as 'Domicilio'
,case 
WHEN Banco.Grupo like 'ANSES%' THEN Sucur.CP
ELSE Banco.CodigoPostal
END as 'CodigoPostal'
,case 
WHEN Banco.Grupo like 'ANSES%' THEN Sucur.Localidad 
ELSE Banco.Localidad
END as 'Localidad'


,concat('cf',Banco.Tarjeta) as 'Barra'
,'VISA Debito' as 'MarcaSistemaFlash'
,case 
when Banco.DestinoSegunCodigoDeEmision like 'AL CLIENT' then 'Domicilio'
when Banco.DestinoSegunCodigoDeEmision like '' then 'Sucursal'
else ''
end as 'Destino'

			from sispoc5_Banco.tarjetasDeDebito as Banco 
			left join sispoc5_Banco.sucursales as Sucur on Sucur.Numero = Banco.CodigoDeSucursal
			where 1
...
,DenominacionDelUsuario as 'Destinatario',
concat(
Calle,
(case when Puerta not like '' then concat(' Numero ',Puerta) else '' end),
(case when Piso not like '' then concat(' Piso ',Piso) else '' end),
(case when NumeroDeDepartamento not like '' then concat(' Departamento ',NumeroDeDepartamento) else '' end)
) as 'Domicilio'
, CodigoPostal as 'CodigoPostal', Localidad as 'Localidad', concat('cf',SecuenciaDeTargeta) as 'Barra'
,case 
when MarcaSistemaFlash = '1' then 'VISA'
when MarcaSistemaFlash = '4' then 'AMEX'
else ''
end as 'MarcaSistemaFlash'
#, MarcaSistemaFlash as 'MarcaSistemaFlash'
,case
when Distribucion like 'K1' then 'Domicilio'
when Distribucion like 'K2' then 'Sucursal'
else ''
end as 'Destino'
...
,ApellidoYNombre as 'Destinatario', CalleYAltua as 'Domicilio', CodigoPostal as 'CodigoPostal', Localidad as 'Localidad', concat('cf',NumeroDeTarjeta) as 'Barra'
#, CodigoDeDestino  as 'Destino'
,case 
when MarcaSistemaFlash = '2' then 'MASTER'
when MarcaSistemaFlash = '3' then 'MAKRO'
else MarcaSistemaFlash
end as 'MarcaSistemaFlash'
,case 
when  SUBSTRING(Telefono, 1, 2) like 'F1' then 'Domicilio'
when  SUBSTRING(Telefono, 1, 2) like 'F2' then 'Sucursal'
else ''
end as 'Destino'
		...
		group by Banco.Tarjeta
		ORDER BY Banco.Tarjeta ASC
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
