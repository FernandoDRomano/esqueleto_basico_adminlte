	...
	$Consulta= "
		...
		left join (
			SELECT 
				pe.Fecha as 'Fecha'
				, case 
					when pe.idEstados is not null 
					then pe.idEstados
					else '49'
				end as 'idEstados'
				, case 
					when pe.idPieza is not null 
					then pe.idPieza
					else pi.PiezaId
				end as 'idPieza'
			FROM sispoc5_gestionpostal.flash_comprobantes_ingresos as ci
			...
			left join sispoc5_Ocasa.PiezasIngresadas as pi on pi.PiezaId = fp.id
			where ci.id = '$ComprobantesDeIngreso' and fp.tipo_id = '2'
		) as pe on fp.id = pe.idPieza
