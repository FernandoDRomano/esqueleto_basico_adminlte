 <?php
	...
	*/
	...
	$Consulta= "
		SELECT hdr.id as 'Id', hdr.id as 'Nombre' #concat('HDR:',hdr.id) as 'Nombre' 
		FROM sispoc5_gestionpostal.flash_hojas_rutas as hdr
		inner join sispoc5_gestionpostal.flash_piezas as p on hdr.id = p.hoja_ruta_id
		inner join sispoc5_gestionpostal.flash_comprobantes_ingresos as ci on p.comprobante_ingreso_id = ci.id
		where hdr.create > (CURRENT_DATE - INTERVAL 120 DAY) and ci.cliente_id = 30 and p.tipo_id = '2'
		group by hdr.id#, p.comprobante_ingreso_id
