<?php
	...
	*/
	...
	$Busqueda=false;
	Busqueda:
	$Columnas = array("Id De Estado","Id De Pieza","Barcode","Estado","Vinculo","Receptor","DNI Receptor","Fecha De Estado","Cartero");
	$Consulta= "
		SELECT PE.id as 'Id De Estado'
			,PE.idPieza as 'Id De Pieza'
			,PE.Barcode as 'Barcode'
			,EEAPP.NombreDeEstado as 'Estado'
			,VD.Nombre as 'Vinculo'
			,PE.ApellidoNombreRecibio as 'Receptor'
			,PE.Documento as 'DNI Receptor'
			#,PE.Fecha as 'Fecha De Estado'
			,PE.Fecha as 'Fecha De Estado'
			,PE.NombreDeUsuario as 'Cartero'
		FROM sispoc5_Banco.PiezasEstados as PE
		left join sispoc5_Ocasa.VinculoDestinatario as VD on PE.$Columna = '$Value' and VD.id = PE.idVinculo 
		inner join sispoc5_Banco.Estados as EEAPP on PE.idEstados = EEAPP.Id
		WHERE PE.$Columna = '$Value'
		...
		if($IdOBarcode == "Id"){
			if(!$Busqueda){
				$Consulta= "
					...
					inner join sispoc5_gestionpostal.flash_piezas on flash_piezas.id = '$Value' and PiezasEstados.Barcode = flash_piezas.barcode_externo
					SET PiezasEstados.idPieza = flash_piezas.id
					where flash_piezas.id = '$Value'
				";
