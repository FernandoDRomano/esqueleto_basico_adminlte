<?php
	...
	*/
	...
	$Columnas = array("Despacho","Sucursal De Origen","Sucursal De Destino","Comprobante De Ingreso","Pieza Id","Barcode","Estado","Estado Activo");
	$Consulta= "
		SELECT pd.id as 'Despacho', fs.nombre as 'Sucursal De Origen', fsd.nombre as 'Sucursal De Destino', fp.comprobante_ingreso_id as 'Comprobante De Ingreso', fp.id as 'Pieza Id', fp.barcode_externo as 'Barcode', peApp.NombreAPP as 'Estado'
		, case pe.Activo
		when '0' then 'No'
		when '1' then  'Si'
		else ''
		...
		left join sispoc5_gestionpostal.flash_sucursales as fs on fs.id = pd.origen_id
		left join sispoc5_gestionpostal.flash_sucursales as fsd on fsd.id = pd.destino_id
