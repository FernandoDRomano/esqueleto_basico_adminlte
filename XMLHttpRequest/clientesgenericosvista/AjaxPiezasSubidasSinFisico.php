<?php
	...
	*/
	...
	$Columnas = array("Destinatario","Domicilio","CodigoPostal","Localidad","Barra","Creacion","ClienteId","ServicioId");
	$Consulta= "
		SELECT 
		#pi.id as 'id',
		...
		pi.Barra as 'Barra',
		pi.Creacion as 'Creacion',
		pi.ClienteId as 'ClienteId',
		pi.ServicioId as 'ServicioId'
		FROM sispoc5_Ocasa.PiezasIngresadas as pi
		left join sispoc5_gestionpostal.flash_piezas as fp on pi.barra = fp.barcode_externo
		where pi.Creacion >= '$FechaI'
		AND pi.Creacion <= '$FechaF'
		and fp.barcode_externo is null
	";
	//echo($Consulta);
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
