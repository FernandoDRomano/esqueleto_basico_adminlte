	...
	*/
	...
	$Consulta= "
		SELECT PE.id as 'Id De Estado'
			...
			,DATE_FORMAT(DATE_ADD(PE.Fecha, INTERVAL 0 HOUR),'%d/%m/%Y %H:%i') as 'Fecha De Estado'
			,PE.NombreDeUsuario as 'Cartero'
		FROM sispoc5_Ocasa.Piezas_Estados as PE
		left join sispoc5_Ocasa.VinculoDestinatario as VD on VD.id = PE.idVinculo
		inner join sispoc5_Ocasa.EstadoEntregaApp as EEAPP on PE.idEstados = EEAPP.id
		WHERE $ColumnaOcasa LIKE '$Value'
