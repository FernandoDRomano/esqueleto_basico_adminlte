<?php
	...
	*/
	...
	$Consulta= "
		select  Barcode as 'Barcode'
		FROM sispoc5_correoflash.BancoMacroStockDePiezas
		WHERE Barcode = '$Barcode' and Estado='1'
		
	";
	//echo($Consulta);
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("La Pieza Fue Quitada Anteriormente");
		exit;
	}else{
	}
	...
	$Consulta= "
		...
		WHERE Barcode = '$Barcode'
		*/
		UPDATE sispoc5_correoflash.BancoMacroStockDePiezas SET Estado='1' WHERE Barcode = '$Barcode'
	";
	...
	$Consulta= "
		select  Barcode as 'Barcode'
		FROM sispoc5_correoflash.BancoMacroStockDePiezas
		WHERE Barcode = '$Barcode' and Estado='0'
