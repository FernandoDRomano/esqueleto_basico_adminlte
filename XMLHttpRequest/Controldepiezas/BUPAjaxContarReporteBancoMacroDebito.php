<?php
	...
	*/
	...
	FROM  (
		...
		inner JOIN sispoc5_Ocasa.Piezas_Estados as E on E.BarcodeExterno = fp.barcode_externo and E.Fecha >= '$FechaI' AND E.Fecha < '$FechaF' AND E.BarcodeExterno like '%cf%'
		...
		inner JOIN 
		(
			SELECT Debito.FechaDeCargaSistemaFlash as 'FechaDeCargaSistemaFlash', Debito.Tarjeta as 'SecuenciaDeTargeta', concat('CF',Debito.Tarjeta) as 'SecuenciaDeTargetaComparar', Debito.CodigoDeSucursal as 'SucursalDeRadicacionDeLaCuenta', Debito.LoteImpreso as 'LoteImpreso'
			FROM sispoc5_Banco.tarjetasDeDebito as Debito
			where 1 #Debito.FechaDeCargaSistemaFlash > (CURRENT_DATE - INTERVAL 90 DAY)
		) as BancoGrabadas on BancoGrabadas.SecuenciaDeTargeta  not like '' and  E.BarcodeExterno not like '' and E.BarcodeExterno = BancoGrabadas.SecuenciaDeTargetaComparar
		...
		inner JOIN sispoc5_gestionpostal.flash_comprobantes_ingresos as fci on  fci.id = fp.comprobante_ingreso_id and fci.cliente_id ='30'
		inner JOIN sispoc5_Ocasa.Piezas_Estados as E on E.BarcodeExterno = fp.barcode_externo and E.Fecha >= '$FechaI' AND E.Fecha < '$FechaF' AND E.BarcodeExterno like '%cf%'
		inner JOIN sispoc5_Ocasa.EstadoEntregaApp as EEA on E.idEstados = EEA.id  and (EEA.id <> '8' AND EEA.id <> '13' AND EEA.id <> '12')
		inner JOIN sispoc5_Banco.EstadoVisa as EV on EEA.idEstado = EV.IdApp
		inner JOIN sispoc5_Banco.EstadosPlasticos as EP on EV.IdPlasticos = EP.Id
		inner JOIN sispoc5_Ocasa.VinculoDestinatario as VCD on VCD.id = E.IdVinculo
		inner JOIN 
		(
			SELECT Debito.FechaDeCargaSistemaFlash as 'FechaDeCargaSistemaFlash', concat('CF',Debito.Tarjeta) as 'SecuenciaDeTargeta', concat('CF',Debito.Tarjeta) as 'SecuenciaDeTargetaComparar', Debito.CodigoDeSucursal as 'SucursalDeRadicacionDeLaCuenta', Debito.LoteImpreso as 'LoteImpreso'
			FROM sispoc5_Banco.tarjetasDeDebito as Debito
			where 1 #Debito.FechaDeCargaSistemaFlash > (CURRENT_DATE - INTERVAL 90 DAY)
		) as BancoGrabadas on BancoGrabadas.SecuenciaDeTargeta  not like '' and  E.BarcodeExterno not like '' and E.BarcodeExterno = BancoGrabadas.SecuenciaDeTargetaComparar
		...
		inner JOIN 
			...
			) as BancoGrabadas on BancoGrabadas.SecuenciaDeTargeta  not like '' and PSpp.Barcode not like '' and PSpp.Barcode = BancoGrabadas.SecuenciaDeTargetaComparar
	...
	#order by Orden.Orden asc #Orden.FECHANOVEDAD,Orden.CARTERENDICION
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
