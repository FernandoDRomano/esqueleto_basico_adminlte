<?php
	...
	*/
	...
	$Id = issetornull('IdUsuario');
	$ClienteUsuario = issetornull('ClienteUsuario');
	$ClientePassword = issetornull('ClientePassword');
	$Password = password_hash($ClientePassword, PASSWORD_DEFAULT);
	$ClientePassword = $Password;
	
	//echo($Password);
	$ClienteNombre = issetornull('ClienteNombre');
	$ClienteNombreFantacia = issetornull('ClienteNombreFantacia');
	$ClienteEmail = issetornull('ClienteEmail');
	
	$Columnas = array("Alias");
	$Consulta= "
		SELECT U.Alias as 'Alias'
		FROM sispoc5_correoflash.cliente as U
		WHERE U.Alias = '$ClienteUsuario'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Cliente " . $ClienteUsuario . " Ya Existe.");exit;
	}
	
	$Columnas = array("Alias");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		SELECT U.Alias as 'Alias'
		FROM sispoc5_correoflash.cliente as U
		WHERE U.Nombre = '$ClienteNombre'
		and U.NombreDeFantacia = '$ClienteNombreFantacia'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Cliente De Apellido: " . $ClienteNombreFantacia . " Y Nombre: ". $ClienteNombre . " Ya Tiene Cuenta, Su Cuenta Es: " . $ClaseMaster->ArraydResultados[0][0]);exit;
	}
	...
	$Consulta= "
		SELECT U.Alias as 'Alias'
		FROM sispoc5_correoflash.cliente as U
		WHERE U.Mail = '$ClienteEmail'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Cliente De Email: " . $ClienteEmail . " Ya Tiene Cuenta, Su Cuenta Es: " . $ClaseMaster->ArraydResultados[0][0]);exit;
	}
	
	/*
	$Columnas = array("");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		ALTER TABLE sispoc5_correoflash.usuario auto_increment = 1;
	";
	...
	*/
	...
	$Consulta= "
		...
		(
			...
			, '$ClienteEmail'
			, '$ClienteNombre'
			, '$ClienteNombreFantacia'
			, '$Id'
			, '1'
			, '0'
			,	(
				SELECT sum(U.idperfil + 1)
	...
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Cliente " . $ClienteUsuario . " Fue Creado.");exit;
	}else{
		echo("El Cliente " . $ClienteUsuario . " No Fue Creado.");exit;
	}
