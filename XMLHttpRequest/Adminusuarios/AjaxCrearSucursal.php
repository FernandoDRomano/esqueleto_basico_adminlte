<?php
	...
	*/
	...
	$SucursalNombre = issetornull('SucursalNombre');
	
	$Columnas = array("Nombre");
	$Consulta= "
		SELECT s.Nombre as 'Nombre'
		FROM sispoc5_correoflash.Sucursales as s
		WHERE s.Nombre = '$SucursalNombre'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Nombre De Sucursal: " . $SucursalNombre . " Ya Existe.");exit;
	}
	...
	*/
	...
	$Columnas = array("");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		INSERT INTO sispoc5_correoflash.Sucursales 
		(
			ID, Nombre, FechaDeCreacion, Creador
		)
		VALUES 
		(
			NULL
	...
	if($Resultado ){
		if($ClaseMaster->Insertado > 0){
			...
			echo("La Sucursal " . $SucursalNombre . " Fue Creado.<br>Busque Sucursales Creadas, (Para Ver Las Nuevas Sucursales).");exit;
		}else{
			echo("La Sucursal " . $SucursalNombre . " No Fue Creado.");exit;
		}
	}else{
		echo("La Sucursal " . $SucursalNombre . " No Fue Creado.");exit;
	}
