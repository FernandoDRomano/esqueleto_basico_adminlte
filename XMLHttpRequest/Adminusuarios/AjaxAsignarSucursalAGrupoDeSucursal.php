<?php
	...
	*/
	...
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("Error:La Sucursal Ya Existe En El Grupo.");exit;
	}
	...
	*/
	...
	$Columnas = array("");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		INSERT INTO sispoc5_correoflash.SucursalesEnGrupos 
		(
			Id, IdGrupoDeSucursales, IdSucursales, FechaDeCreacion, Creador
		)
		VALUES 
		(
			NULL
	...
	if($Resultado ){
		if($ClaseMaster->Insertado > 0){
			...
			echo("La Sucursal Fue Insertada En El Grupo.<br>Busque Sucursales En Grupos De Sucursales, (Para Ver Compocicion De Grupos De Sucursales).");exit;
		}else{
		echo("La Sucursal No Se Inserto En El Grupo.");exit;
		}
	}else{
		echo("La Sucursal No Se Inserto En El Grupo.");exit;
	}
