<?php
	...
	*/
	...
	$MenuNombre = issetornull('MenuNombre');
	$MenuCarpetaRaiz = issetornull('MenuCarpetaRaiz');
	$MenuVistaDeMenu = issetornull('MenuVistaDeMenu');
	...
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Nombre De Menu: " . $MenuNombre . " Ya Existe.");exit;
	}
	
	$Columnas = array("Nombre");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		SELECT CFM.Nombre as 'Nombre'
		#, CFM.URL as 'MenuCarpetaRaiz'
		#, CFM.MainMenu as 'Vista De Menu'
		...
		#, CFM.Creador as 'U.IdCreador'
		FROM sispoc5_correoflash.menu as CFM
		WHERE CFM.URL = '$MenuCarpetaRaiz'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("La Carpeta Raiz: " . $MenuCarpetaRaiz . " Ya Existe Para El Menu: " . $ClaseMaster->ArraydResultados[0][0]);exit;
	}
	...
	*/
	...
	$Columnas = array("");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		INSERT INTO sispoc5_correoflash.menu 
		(
			id, Nombre, URL, MainMenu, FechaDeCreacion, Creador
		)
		VALUES
		(
			NULL
			, '$MenuNombre'
			, '$MenuCarpetaRaiz'
			, 'Views/$MenuVistaDeMenu.php'
			, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL '$DiferenciaSQL' HOUR_MINUTE)
			,	(
				SELECT U.idperfil
