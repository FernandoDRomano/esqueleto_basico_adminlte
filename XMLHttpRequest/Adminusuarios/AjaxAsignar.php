<?php
	...
	*/
	...
	$Menues = issetornull('Menues');
	$Usuario = issetornull('Usuario');
	
	
	
	$Columnas = array("Id","Usuario");
	$Consulta= "
		SELECT CFM.IdUsuario as 'Id', CFU.us_name as 'Usuario'
		#CFU.idusuario, CFU.us_name, CFU.us_password, CFU.us_mail, CFU.us_nombre, CFU.us_apellido, CFU.FechaDeCreacion, CFU.IdCreador, CFU.us_estado, CFU.us_codseg, CFU.idperfil
		FROM sispoc5_correoflash.usuario as CFU
		#id, IdUsuario, IdMenu, Creador, FechaDeCreacion 
		inner join sispoc5_correoflash.menudeusuarios as CFM on CFU.idusuario = CFM.IdUsuario
		WHERE CFM.IdUsuario = '$Usuario' and CFM.IdMenu = '$Menues'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Usuario: " . $ClaseMaster->ArraydResultados[0][1] . " Ya Tiene Este Menu");exit;
	}
	...
	$Columnas = array("");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		ALTER TABLE sispoc5_correoflash.menudeusuarios auto_increment = 1;
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,false);
	*/
	...
	if($Resultado){
		echo("Menu Agregado A Usuario: " . $ClaseMaster->ArraydResultados[0][1] . "");exit;
	}else{
		echo("No Se Pudo Agregar El Menu Seleccionado");exit;
	}
