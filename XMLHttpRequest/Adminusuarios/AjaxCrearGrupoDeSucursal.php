<?php
	...
	*/
	...
	$GrupoDeSucursalNombre = issetornull('GrupoDeSucursalNombre');
	
	$Columnas = array("Nombre");
	$Consulta= "
		SELECT gds.NombreDeGrupo as 'Nombre'
		FROM sispoc5_correoflash.GrupoDeSucursales as gds
		WHERE gds.NombreDeGrupo = '$GrupoDeSucursalNombre'
	";
	$Resultado = $ClaseMaster->SQL_Master($Consulta,$Columnas,$time,true);
	if($Resultado){
		echo("El Nombre De Sucursal: " . $GrupoDeSucursalNombre . " Ya Existe.");exit;
	}
	...
	*/
	...
	$Columnas = array("");//,"Password","Email","Nombre","Apellido","Fecha De Creacion"
	$Consulta= "
		INSERT INTO sispoc5_correoflash.GrupoDeSucursales 
		(
			Id, NombreDeGrupo, FechaDeCreacion, IdCreador
		)
		VALUES 
		(
			NULL
			, '$GrupoDeSucursalNombre'
			, CURRENT_TIMESTAMP	
			, '$IdUsuario'
		)
	";
	...
	if($Resultado ){
		if($ClaseMaster->Insertado > 0){
			...
			echo("El Grupo De Sucursales " . $GrupoDeSucursalNombre . " Fue Creado.<br>Busque Grupos De Sucursales, (Para Ver Los Nuevos Grupos De Sucursales).");exit;
		}else{
			echo("El Grupo De Sucursales " . $GrupoDeSucursalNombre . " No Fue Creado.");exit;
		}
	}else{
		echo("El Grupo De Sucursales " . $GrupoDeSucursalNombre . " No Fue Creado.");exit;
	}
