<?php
	...
	*/
	...
	$Consulta= "
		SELECT count(distinct(M.id)) as 'Contador'
			, M.Nombre as 'Nombre'
			, M.URL as 'Carpeta Raiz'
			, M.MainMenu as 'Vista De Menu'
			, M.FechaDeCreacion as 'Fecha De Creacion'
			, M.Creador as 'U.IdCreador'
			from sispoc5_correoflash.menu as M
			WHERE 1
		...
		$Consulta= "
			SELECT count(distinct(M.id)) as 'Contador'
				...
				inner join sispoc5_correoflash.menu as M on (M.id = MDU.IdMenu)
				WHERE (U.idusuario = '$IdUsuario' or '$IdUsuario' = '1')
				order by M.Nombre asc
