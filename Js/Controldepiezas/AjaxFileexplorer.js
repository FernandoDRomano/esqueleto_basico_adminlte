...
function AddHeight() {
	$(".search").css("height", $(".breadcrumbs").height()+4);
	$(".search").css("line-height", $(".breadcrumbs").height() + 'px');
};

var FicheroDeOrigen = "";// #nombreDeFichero Sera Agregado Para La Ruta
var NombreFicheroDeOrigen = "fileexplorer";// Main Ref -> 
var Directorio="cliente_flash_reportes";// Main Ref -> 
var DirectorioInicial="cliente_flash_reportes";// Main Ref -> 
if(!SinSobdominio){
	var DirectorioDescarga="";// Main Ref -> 
}
var DirectorioDescarga="";// Main Ref -> 
var RutaDeBusqueda = '..\/cliente_flash_reportes\/files';
var scanerphp = URLJS + 'XMLHttpRequest/'+'scan.php';
...
$(function(){
	...
	$.get(scanerphp + '?dir=' + RutaDeBusqueda + '&time='+FechaCompleta, function(data) {
			...
			if(scannedFiles.length) {
				scannedFiles.forEach(function(f) {
					...
					switch (fileType.toLowerCase()) {
						...
						break;
						case "txt":
							icon = '<img style="display: block;max-height: 90px;line-height: 3em;text-align: center;border-radius: 0.25em;color: #FFF;display: inline-block;margin: 0.9em 1.2em 0.8em 1.3em;position: relative;overflow: hidden;" src="../IMAGENES/DocumentoDeTexto.png" alt="">';
						break;
						case "mp4":
					...
					var file = $('<li class="files"><a href="'+ DirectorioDescarga + f.path+'" target="_blank" title="'+ f.path +'" class="files" download>'+icon+'<span class="name">'+ name +'</span> <span class="details">'+fileSize+'</span></a></li>');
			...
			if(Folders.hasClass('searching')){
				...
				breadcrumbsUrls.forEach(function (u, i) {
					...
					if (i !== breadcrumbsUrls.length - 1) {
						if(name[name.length-1]!=".." && name[name.length-1] != DirectorioInicial){
							url += '<a href="'+u+'"><span class="folderName">' + name[name.length-1] + '</span></a> <span class="arrow">→</span> ';
						}else{
							url += '<a href="'+u+'" style="display:none;"><span class="folderName">' + name[name.length-1] + '</span></a> <span class="arrow" style="display:none;" >→</span> ';
						}
						//
					}
					else {
